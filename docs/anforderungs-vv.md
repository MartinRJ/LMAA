...
| INT-001 | LMAA ist installiertes `ACTION_SEND`-/`text/plain`-Ziel für YouTube; die bevorzugte Samsung-Sharesheet-Position bleibt Nutzer-/Systemkonfiguration | **validiert**; der App-Vertrag und die von One UI verwaltete Rangfolge sind getrennte Verantwortlichkeiten. Ein künstlicher Direct-Share-Kontakt ohne Kommunikationshistorie passt fachlich nicht zu LMAA | **erfüllt**; gemergtes Release-Manifest und Laufzeit akzeptieren ausschließlich `ACTION_SEND` mit `text/plain`. JVM-Test, Lint, signierter Build und isolierter Android-13-PackageManager-Test bestehen; die installierte Release-App löst positiv für `text/plain` und negativ für `text/html`, Bilder sowie `ACTION_VIEW` auf. Der erfolglose Direct-Share-Shortcut ist entfernt und der System-Shortcut-Store leer. Der Nutzer hat LMAA anschließend über Samsung Good Lock erfolgreich neben Quick Share, Telegram und Outlook angeheftet | Exakten Manifest-/Resolververtrag in der Regression beibehalten; Sharesheet-Favorit bleibt Nutzerkonfiguration |
| TST-001 | Tests dürfen tägliche BYOK-/Historiedaten nicht zerstören | **validiert**; persönliche App-Daten sind produktive Stakeholderdaten | **erfüllt**; Default-Testvariante `instrumented` nutzt `de.lmaa.app.testbed`. Vierzehn Instrumentierungstests liefen isoliert auf dem Zieltablet; danach wurden ausschließlich die Testbed-Pakete entfernt. Der Live-BYOK-Test ist standardmäßig durch eine explizite Annahme gesperrt | Isolation in CI und bei künftigen Gerätetests beibehalten |
