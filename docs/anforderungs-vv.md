...
| FAL-001 | RapidAPI nur nach geeignetem Primärfehler und Opt-in | **validiert**; schont das persönliche Kontingent | **erfüllt**; Android-Adapter und Wahrheitstabelle erzwingen Opt-in, Key, Fehler-Whitelist, exakt einen Request und keinen 429-Retry; eigene Settings-Ansicht speichert RapidAPI-BYOK verschlüsselt, deaktiviert beim Löschen atomar und zeigt den lokalen Monatszähler. Nur `/100` ist ein hellgrauer, bedingter Basic-Hinweis; Dashboard ist maßgeblich | realer Fallback nur bei diagnostischem Bedarf |
...
| TST-001 | Tests dürfen tägliche BYOK-/Historiedaten nicht zerstören | **validiert**; persönliche App-Daten sind produktive Stakeholderdaten | **erfüllt fortlaufend**; `adb install -r` plus zehn gezielte Instrumentierungstests statt UTP erhielten BYOK-Maske und fünf Historieneinträge; ausschließlich erzeugte M3-Testdaten wurden wieder gelöscht | isolierte Test-Application-ID bleibt M4-Härtung |
